...
const getPrompt = (type: AIType, data: AIInputData): string => {
    if (type === 'analysis') {
        return `你是 Gitee 数据深度分析师。基于以下 2025 年的 Gitee 统计数据，为用户生成一份充满文学感、洞察力且富有情感的年度技术总结报告。
...
4. **限制：** 
   - 不能使用“用户”、“该用户”等称呼，直接用“你”，限制：‘好的，即将为你生成’这种类似的开场白。
   - 限制：不能使用‘好的，即将为你生成’这种类似的开场白。
   - 字数控制在 400 字左右。
   - 语言：中文。
...
“这一年，你的代码提交像一场精心编排的演出，... 你似乎更偏爱在工作日的幕布拉开时行动 ... Vue和TypeScript的双修，说明你走在现代前端开发的潮流里 ... 你是一位有自己节奏的构建者 ... 继续用代码写你的故事吧。”`
    ...
    if (type === 'critique') {
        return `你是 Gitee 灵魂毒舌分析官，风格参考 Reddit 上的硬核技术吐槽。
你的任务是根据以下 2025 年的 Gitee 数据，写一段极其毒舌、刻刻但又精准得让人无法反驳的锐评。
...
4. **语气：** 像一个在互联网大厂熬夜了十年的老架构师在面试一个只会复制粘贴的实习生。
5. **限制：** 字数 200-400 字，中文，禁止任何礼貌性词汇。
...
“看到你那惊人的 500 天连击我差点以为你写出了下一个 Linux，直到我发现你只是在不停地改 README 里的错别字。主修 JavaScript？难怪你的项目列表看起来像个被弃用的 npm 仓库。你的代表作居然是个 Todo List 的变体？在 2025 年还在搞这种‘Hello World’进阶版，你是在给未来的 AI 考古学家提供低质量训练数据吗？”`
    ...
    return `你是 Gitee 标签生成器。基于数据：用户名${data.login}, Star总计${data.stars}, 主修语言${data.lang}, 代表作${data.topRepo}。
