    ...
    <div>
        ...
        <div v-if="currentPage === 'landing'" id="landingPage" class="min-h-screen flex flex-col items-center px-4 py-12 md:py-24">
            ...
            <footer class="text-center pt-12 text-slate-500 text-xs space-y-2">
                <p>© Gitee 2025 Year in Code. Built with ❤️ for Developers.</p>
                <p class="opacity-50 font-mono tracking-wider flex items-center justify-center gap-2">
                    ...
                    <a href="https://gitee2025.lz-t.top/" class="hover:text-teal-400 transition-colors">gitee2025.lz-t.top</a>
                </p>
            </footer>
        </div>
...
import type { UserData, GiteeUser, GiteeRepo } from './types'
...
const startAnalysis = async (username: string) => {
    ...
    if (!username.trim()) {
        showToast('请输入有效的 Gitee ID', 'error')
        return
    }
    ...
    try {
        // 1. 获取用户信息
        const userRes = await fetch(`https://gitee.com/api/v5/users/${username}`)
        if (userRes.status === 403) {
            throw 'Gitee API 速率达到上限。请稍后再试。'
        }
        const user = await userRes.json()
        if (userRes.status === 404) throw '未找到该用户'
        ...
        let page = 1
        while (page <= 5) {
            const r = await fetch(`https://gitee.com/api/v5/users/${username}/repos?per_page=100&page=${page}`)
            ...
            if (!d || d.length === 0 || !Array.isArray(d)) break
        ...
        const baseContributions = repos2025.length * 20 + followers * 2
        ...
        // 技术领袖系列
        if (totalStars > 10000 && followers > 5000) {
            rank = '开源传奇'
        } else if (totalStars > 5000 && followers > 1000) {
            rank = '年度技术领袖'
        } else if (totalStars > 2000 && followers > 500) {
            rank = '社区领袖'
...
const downloadPoster = async () => {
    ...
    try {
        ...
        link.download = `Gitee-Trace-2025-${userData.value.login || 'user'}.png`
        link.href = dataUrl
