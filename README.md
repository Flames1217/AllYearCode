# 🌌 Gitee Trace Soul - 2025 年度技术溯源

> **Trace Your Code Soul.** 深度解析 Gitee 开发者行为与资产配置，接入 Mimo AI 大模型，生成专属开发者的赛博灵魂名片。
...
-   **内存存储**：用户输入的 Gitee Token 仅存储在浏览器的运行时内存中，不会持久化到 LocalStorage 或 Cookie。
-   **直连 Gitee**：所有涉及 Gitee 数据（REST/GraphQL）的请求均由浏览器直连 Gitee 官方服务器，不经过任何第三方中转。
...
-   **性能优化**：对 Gitee API 调用进行了全量异步递归处理，并针对图表实例进行了自动销毁管理，防止内存泄漏。
